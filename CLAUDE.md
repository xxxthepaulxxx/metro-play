...
**Module 4:** 城市 RPG 解鎖 (City RPG Unlock) — unlock Taipei districts by accumulating station visits on an adventure map; PRD at docs/product-requirement-document/metro-city-rpg-unlock/requirement.md
...
**Module 4 (城市 RPG 解鎖):** Amber (#FF8F00) as accent for exploration/adventure warmth. AdventureMapCard uses glassmorphism grid with amber border-glow on unlocked districts; locked districts use dimmed opacity (0.4) + lock icon overlay. DistrictDetailCard shows station checklist with amber checkmarks for visited stations. UnlockAnimation is one-shot radial-burst (500ms) + amber glow ring (400ms). Tokens: --color-rpg-accent, --color-rpg-accent-dim, --color-rpg-accent-border, --color-rpg-accent-glow.
